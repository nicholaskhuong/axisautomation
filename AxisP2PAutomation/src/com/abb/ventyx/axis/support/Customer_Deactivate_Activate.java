...
public class Customer_Deactivate_Activate extends BaseTestCase {
	...
	public void clickFiterButtonOnCustomerScreen() {
		...
		WebElement filterPermissionName = (new WebDriverWait(driver, 30))
				.until(ExpectedConditions.presenceOfElementLocated(By.xpath(CustomerList.EMAIL_FILTER)));
	...
	//	assertEquals(action.getTextField(By.id(CustomerList.STATUS)), "Inactive");
	}
	...
	public void logoutAndLoginWithMail5() {
		action.signIn(emailAdmin, passwordAdmin);
		action.waitObjVisibleAndClick(By.cssSelector(AxisConfigMenu.CUSTOMER_MAINTENANCE));
		action.waitObjVisibleAndClick(By.cssSelector(AxisConfigMenu.CUSTOMER_LIST));
	}
	
	@Test(dependsOnMethods = "logoutAndLoginWithMail5", alwaysRun = true)
